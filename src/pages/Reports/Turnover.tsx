...
// Lean and ultra polished compact KPI Card
          ...
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 bg-gradient-to-tr from-[#b58c4f] to-[#f3e5ab] rounded-xl flex items-center justify-center text-[#212c46] shadow-inner p-[2px]">
                <div className="w-full h-full bg-[#0F172A] rounded-[10px] flex items-center justify-center">
                    <TrendingDown size={18} className="text-[#b22026]"/>
                </div>
            </div>
            <div>
              <h3 className="text-sm font-black flex items-center gap-2 uppercase tracking-widest leading-none mb-1 drop-shadow-sm">TURNOVER ANALYSIS GUIDE</h3>
              <p className="text-[10px] font-bold text-[#748b9e] uppercase tracking-widest mt-1 drop-shadow-sm">คู่มือวิเคราะห์อัตราการลาออกและเสถียรภาพองค์กร</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6 text-[#414757] text-[12px] leading-relaxed bg-white">
          <section className="animate-fadeIn">
            <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <BookOpen size={16} className="text-[#b22026]"/> 1. Attrition Registry Node (ระบบศูนย์ข้อมูลการลาออก)
            </h4>
            <p className="mb-2 text-[#414757] font-medium text-[11.5px]">แดชบอร์ดรวบรวมเหตุผล ข้อมูลอายุงาน และสถิติต่างๆ ของการลาออก รายสัปดาห์ / รายเดือน:</p>
            <ul className="space-y-2 list-none pl-0 text-[11px] font-medium text-[#606a5f]">
              <li className="flex items-start gap-2 bg-[#f8f9fa] p-2.5 rounded-xl border border-[#eaeaec]">
                ...
                <div>
                  <strong>Personnel Record:</strong> บันทึกประวัติพนักงานที่แจ้งความจำนงลาออกอย่างชัดเจนเป็นรายบุคคล ทั้งตำแหน่ง แผนก และอายุงานสะสม
                </div>
              </li>
              <li className="flex items-start gap-2 bg-[#f8f9fa] p-2.5 rounded-xl border border-[#eaeaec]">
                <Target size={16} className="shrink-0 text-[#b58c4f] mt-1"/>
                <div>
                  <strong>Exit Interview Root Cause:</strong> บันทึกเหตุผลการลาออกที่สำคัญเพื่อใช้วิเคราะห์ปัญหาภายใน (เช่น สภาพแวดล้อม ค่าจ้าง ความสัมพันธ์)
                </div>
              </li>
            </ul>
          </section>
          ...
          <section className="animate-fadeIn" style={{ animationDelay: '0.1s' }}>
            <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <BrainCircuit size={16} className="text-[#b22026]"/> 2. AI Predictive Analytics (การเชื่อมโยงระบบพยากรณ์)
            </h4>
            <div className="bg-[#fff5f5] p-3.5 rounded-2xl border border-[#ffe3e3] space-y-2 text-[11px] text-[#606a5f] font-medium">
              <p className="text-[#b22026] font-black uppercase flex items-center gap-1.5"><ShieldAlert size={14}/> Predictive Sync Activation:</p>
              <p>• <strong>Active Sync:</strong> ข้อมูลสถิติของพนักงานจะถูกซิงค์เข้าระบบตรวจจับเชิงลึก เพื่อประเมินดัชนีชี้วัดเสถียรภาพแผนก และพยากรณ์โอกาสลาออกของตำแหน่งงานสำคัญ</p>
              <p>• <strong>Retention Rate Impact:</strong> คำนวณร้อยละความมั่นคงบุคคล เพื่อเสนอมาตรการบำรุงรักษาและจูงใจพนักงานได้ทันเวลา</p>
            </div>
          </section>

          <section className="animate-fadeIn" style={{ animationDelay: '0.2s' }}>
            <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Key size={16} className="text-[#b58c4f]"/> 3. Configuration Management (โครงสร้างการตั้งค่า)
            </h4>
            <div className="grid grid-cols-1 gap-2.5">
              {[
                { icon: Edit3, label: 'Standard Configuration Node', desc: 'หน้าต่างแก้ไขจัดกลุ่ม 3 ด้าน (Profile, Details, AI Sync) แบบเป็นขั้นเป็นตอนระดับเดียวกับ User Permissions', color: '#b58c4f' },
                { icon: Trash2, label: 'Archive/Delete Protocol', desc: 'ความเสี่ยงด้านความเสถียรข้อมูลจะถูกจำกัดเฉพาะผู้ดูแลและผู้ได้รับสิทธิ์เข้าถึงเท่านั้น', color: '#b22026' },
              ].map((btn, idx) => (
          ...
          <section className="animate-fadeIn" style={{ animationDelay: '0.3s' }}>
             <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
               <RefreshCw size={16} className="text-[#3f809e]"/> 4. Audit & Compliance
             </h4>
             <p className="bg-[#3f809e]/5 text-[#2f2926] p-3 rounded-xl border border-[#3f809e]/20 font-medium text-[11px] leading-relaxed">
               ภายใต้ข้อกำหนด PDPA ข้อมูลสิทธิส่วนบุคคลการลาออกและ Exit Interview จะถือเป็นระดับความลับส่วนบุคคลระดับสูงที่จะมีเพียงสิทธิ์ที่เกี่ยวข้องเท่านั้นที่จะเข้ารหัสวิเคราะห์เชิงลึกได้
             </p>
          </section>
        </div>
      ...
      <div className="px-4 sm:px-8 w-full mt-[2px]">
        <div className="w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-3 shrink-0">
                <KpiCard label="YTD Attrition" value={records.length} icon={UserMinus} colorAccent={THEME.palette.brick} desc="Active Exit Cases" />
                <KpiCard label="Turnover Rate" value="4.2%" icon={TrendingDown} colorAccent={THEME.palette.red} desc="Annual Forecast" />
                <KpiCard label="Avg Experience" value="3.5 Yrs" icon={Clock} colorAccent={THEME.palette.slate} desc="Workforce Tenure" />
                <KpiCard label="Retention Rate" value="95.8%" icon={TrendingUp} colorAccent={THEME.palette.forest} desc="Healthy Standard" />
            </div>
