...
// --- Premium Lean Local KPI Card matching home aesthetic but with compact padding ---
const LocalKpiCard = ({ icon: IconComponent, value, label, color, description }: any) => (
    <div className="bg-white p-4 rounded-2xl border border-[#eaeaec] shadow-sm flex-1 min-w-[200px] relative overflow-hidden group hover:border-[#709654] transition-all flex flex-col justify-between animate-fadeIn font-sans">
        <div className="absolute -right-4 -bottom-6 opacity-[0.05] transform group-hover:scale-110 transition-transform duration-75 pointer-events-none">
            <IconComponent size={90} style={{ color }} />
        </div>
        <div className="relative z-10 flex justify-between items-start w-full gap-2">
            <span className="text-[10px] font-black text-[#7a8b95] uppercase tracking-[0.08em] truncate max-w-[85%]">{label}</span>
            <div className="w-8 h-8 rounded-lg border flex items-center justify-center shrink-0 shadow-sm transition-all group-hover:rotate-6" style={{ backgroundColor: `${color}15`, borderColor: `${color}25`, color }}>
                <IconComponent size={14} />
            </div>
        </div>
        <div className="relative z-10 mt-2 flex items-baseline justify-between w-full">
            <div className="text-[20px] font-black leading-none text-[#212c46] font-sans" style={{ color }}>
                {value}
            </div>
            {description && (
                <span className="text-[10px] font-bold text-[#7a8b95] uppercase tracking-widest flex items-center gap-1.5 shrink-0 select-none">
                    <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ backgroundColor: color }}></span> {description}
                </span>
            )}
        </div>
    </div>
...
// --- In-depth user onboarding guide panel matching user permissions standard ---
  ...
  return createPortal(
    <>
      ...
      <div className={`fixed inset-y-0 right-0 z-[200] w-full md:w-[500px] bg-white shadow-2xl transform transition-transform duration-500 ease-in-out flex flex-col border-l-4 border-[#709654] ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex justify-between items-center p-5 px-6 border-b-2 border-[#709654] bg-[#222b38] text-white shrink-0 font-sans">
          <div>
            <h3 className="font-black flex items-center gap-3 uppercase tracking-widest text-[14px] leading-none mb-1.5"><Icons.BookOpen size={18} className="text-[#709654]"/> ONBOARDING DIRECTIVE</h3>
            <p className="text-[11px] font-bold text-[#d7d7d7] uppercase tracking-widest leading-none mt-1">กระบวนการรับและประเมินผลพนักงานใหม่อย่างละเอียด</p>
          </div>
          <button onClick={onClose} className="p-2 text-white/50 hover:text-[#932c2e] hover:bg-white/10 rounded-xl transition-colors"><Icons.X size={20}/></button>
        </div>
        
        <div className="flex-1 overflow-y-auto p-6 space-y-6 text-[#414757] text-[12.5px] leading-relaxed custom-scrollbar bg-white font-sans">
          <section className="animate-fadeIn">
            <h4 className="text-[13px] font-black text-[#212c46] mb-2.5 uppercase flex items-center gap-2 border-b-2 border-[#eaeaec] pb-1.5 font-mono">
              <Icons.ShieldAlert size={16} className="text-[#b58c4f]"/> 1. บทนำความเข้ากันได้ระบบ (Introduction)
            </h4>
            <p className="text-[12px] text-[#606a5f]">
              โมดูลการบริหารจัดการพนักงานใหม่ <b>(Talent Onboarding Hub)</b> เป็นศูนย์รวมกระบวนการที่เชื่อมโยงตรงจากระบบงานรับสมัคร (Recruitment) เพื่อเปลี่ยนสถานะว่าที่พนักงานผู้ผ่านการคัดเลือกเข้าจุดเริ่มต้นทดลองงาน โดยติดตามความพร้อมในด้านเครื่องมือ ระบบ และระเบียบปฏิบัติ (Day-1 Readiness Checklist) ได้อย่างโปร่งใส 100%
            </p>
          </section>
          
          <section className="animate-fadeIn" style={{ animationDelay: '0.1s' }}>
            <h4 className="text-[13px] font-black text-[#212c46] mb-2.5 uppercase flex items-center gap-2 border-b-2 border-[#eaeaec] pb-1.5 font-mono">
              <Icons.LayoutGrid size={16} className="text-[#3f809e]"/> 2. รายการตรวจสอบสิทธิ์ความพร้อม (Checklist Nodes)
            </h4>
            <p className="text-[12px] mb-2 font-bold text-[#606a5f]">ระบบตรวจทานความพร้อมจัดกระบวนการออกเป็น 3 โหนดงานหลัก ดังนี้:</p>
            <ul className="list-disc pl-5 space-y-2 text-[12.5px]">
                <li><strong className="text-[#212c46]">HR, Docs & Benefits (ระเบียบและสัญญา):</strong> การลงนามจัดทำสัญญาจ้างพนักงาน การทรานสคริปต์สิทธิ์ ถ่ายภาพจัดทำบัตร จัดส่งลายนิ้วมือ/ใบหน้าเบื้องต้น สิทธิสวัสดิการ และการขึ้นทะเบียนจัดตั้งประกันสังคมกลุ่ม</li>
                <li><strong className="text-[#212c46]">IT Hardware & Software Access (คอมพิวเตอร์และอีเมล):</strong> การส่งมอบเครื่องจักร/Laptop ประจำตัวพนักงาน จัดทำบัญชีอีเมลองค์กร สิทธิ์เข้าถึงเครื่องมือ ERP-HRMS ภายใน และกำหนดสิทธิ์โปรแกรมซอฟต์แวร์</li>
                <li><strong className="text-[#212c46]">Orientation & Culture (พี่เลี้ยงและการจัดทีม):</strong> กิจกรรมปฐมนิเทศกฎบริษัทอย่างใกล้ชิด การนำเที่ยวรอบสำนักงาน แนะนำสมาชิกในทีม ดำเนินการแต่งตั้งคู่พี่เลี้ยง (Buddy Node) ในช่วงทดลองงาน</li>
            </ul>
          </section>

          <section className="animate-fadeIn" style={{ animationDelay: '0.2s' }}>
            <h4 className="text-[13px] font-black text-[#212c46] mb-2.5 uppercase flex items-center gap-2 border-b-2 border-[#eaeaec] pb-1.5 font-mono">
              <Icons.Award size={16} className="text-[#709654]"/> 3. ฟอร์มประเมินการทดลองงาน (Probation Scorecard)
            </h4>
            <p className="text-[12px] text-[#606a5f] leading-normal">
              แบบประเมินผลการทดลองงานมีค่าเริ่มต้น <b>119 วัน</b> (สามารถแก้ไขจำนวนวันได้ ระบบจะคำนวณวันสิ้นสุดโดยอัตโนมัติ) มาพร้อมเกณฑ์ประเมินทักษะ 4 ด้าน (เต็ม 10 คะแนน): 
            </p>
            <ul className="list-disc pl-5 mt-2 space-y-1 text-[12px]">
              <li><strong className="text-[#212c46]">Work Quality (ผลงาน):</strong> คุณภาพ ประสิทธิภาพ และความถูกต้องในวิชาชีพที่ได้รับมอบหมาย</li>
              <li><strong className="text-[#212c46]">Attitude & Team (ทัศนคติ):</strong> การทำงานร่วมหมู่คณะ และการเปิดรับการเรียนรู้สิ่งใหม่</li>
              <li><strong className="text-[#212c46]">Attendance (เวลาทำงาน):</strong> ความสม่ำเสมอ ความรับผิดชอบ และการตรงต่อเวลา</li>
              <li><strong className="text-[#212c46]">Compliance (วินัย):</strong> การรักษาวินัย กฎจราจร ความปลอดภัย และจารีตองค์กร</li>
            </ul>
          </section>

          <section className="animate-fadeIn" style={{ animationDelay: '0.3s' }}>
            <h4 className="text-[13px] font-black text-[#212c46] mb-2.5 uppercase flex items-center gap-2 border-b-2 border-[#eaeaec] pb-1.5 font-mono">
              <Icons.RefreshCw size={16} className="text-[#d96245]"/> 4. การบูรณาการข้ามพอร์ตและโอนเข้าทำเนียบจริง (Roster Integration)
            </h4>
            <p className="text-[12px] text-[#606a5f]">
              เมื่อพนักงานใหม่เตรียมการจนครบหรือได้รับเกณฑ์ประเมินทดลองงานเรียบร้อยเป็น <b>Passed</b> ปุ่มพิเศษ <b>"โอนข้อมูลพนักงาน" (Transfer to Directory)</b> จะเปิดสิทธิ์ให้สามารถส่งถ่ายชื่อ ภาพถ่าย สังกัดตำแหน่ง เข้าบัญชีทะเบียนพนักงานกลาง <b>Employee Directory</b> ได้ทันทีโดยอัตโนมัติ เพื่อนำไปคำนวณบันทึกเข้างาน เงินเดือน หรือสวัสดิการในระบบถัดไปแบบครบวงจร
            </p>
          </section>
        </div>
        
        <div className="p-4 bg-[#f8f9fa] border-t border-[#eaeaec] flex justify-end shrink-0">
          <button onClick={onClose} className="px-8 py-2 bg-[#212c46] hover:bg-[#414757] text-white font-black rounded-xl uppercase text-[12px] transition-all shadow-md tracking-[0.1em] cursor-pointer">รับทราบ (Got it)</button>
        </div>
      </div>
    </>, document.body
  );
        ...
        >
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden bg-[#f8f9fa] min-h-[460px]">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 bg-white">
                    ...
                    {modalStep === 3 && (
                        <div className="space-y-4 max-w-xl animate-fadeIn font-sans">
...
export default function Onboarding() {
  ...
  return (
    <div className="flex flex-1 w-full flex-col animate-fadeIn bg-transparent space-y-4 px-4 sm:px-8 py-6 mt-[2px] font-sans">
