...
// --- In-depth user onboarding/offboarding guide panel matching user permissions standard ---
    ...
    <>
      ...
      <div className={`fixed inset-y-0 right-0 z-[200] w-full md:w-[500px] bg-white shadow-2xl transform transition-transform duration-500 ease-in-out flex flex-col border-l-2 border-[#b7a159] ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex justify-between items-center p-5 px-6 border-b-2 border-[#b7a159] bg-[#212c46] text-white shrink-0">
          <div>
            <h3 className="font-black flex items-center gap-3 uppercase tracking-widest text-lg"><Icons.BookOpen size={22} className="text-[#b7a159]"/> OFFBOARDING DIRECTIVE</h3>
            <p className="text-[12px] font-bold text-[#d7d7d7] uppercase tracking-widest mt-1.5">ระเบียบและแนวทางการตรวจสอบคืนระบบ</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 text-[#414757] text-[12px] leading-relaxed custom-scrollbar bg-white">
          <section className="animate-fadeIn">
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Icons.ShieldAlert size={18} className="text-[#b7a159]"/> 1. วัตถุประสงค์ (Clearance Purpose)
            </h4>
            <p className="text-[12px] mb-3">
              กระบวนการจำหน่ายพนักงานพ้นสภาพ <b>(Talent Offboarding Clearance)</b> ดำเนินงานเพื่อคุ้มครองความปลอดภัยโครงสร้างเครือข่าย เทคโนโลยี และความโปร่งใสทางการเงินของบริษัท ชัยศรีอะโกรอินดัสเทรียล การอนุมัติพ้นสภาพอย่างสมบูรณ์แบบ 100% ต้องผ่านความเห็นชอบครบสามฝ่ายงานหลัก
            </p>
          </section>
          
          <section className="animate-fadeIn" style={{ animationDelay: '0.1s' }}>
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Icons.Key size={18} className="text-[#d96245]"/> 2. ฝ่ายตรวจสอบความโปร่งใส (Clearance Chains)
            </h4>
            <p className="text-[12px] mb-3">การตรวจสอบและการคืนสิทธิ์ข้อมูล แบ่งออกเป็น 3 ฝ่ายที่ห้ามข้ามขั้นตอน:</p>
            <ul className="list-disc pl-5 mt-2 space-y-2 text-[12px]">
                <li><strong className="text-[#4d87a8]">IT Infrastructure Team:</strong> ยึดคืนฮาร์ดแวร์ประจำกาย สำรองความจุเอกสารส่วนตัวพนักงาน และปิดบัญชี ERP/Email ถาวรทันวันสุดท้าย</li>
                <li><strong className="text-[#657f4d]">HR Officer & Support:</strong> ดึงบัตรพนักงาน เก็บกุญแจส่วนบุคคล เช็คชุดประจำตำแหน่ง จัดส่งหนังสือผ่านงาน ลบสิทธิ์ประกันกลุ่มพร้อมปรับสถานะประกันสังคมพ้นงาน</li>
                <li><strong className="text-[#b58c4f]">Finance & Treasury Node:</strong> หักล้างส่วนเศษใบงวดสะสม วิจัยยอดจ่ายเงินทดรองทางการค้าและสรุปประมวลผลเงินได้ค้างจ่าย (Prorated calculations)</li>
            </ul>
          </section>

          <section className="animate-fadeIn" style={{ animationDelay: '0.2s' }}>
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Icons.Users size={18} className="text-[#3f809e]"/> 3. การสัมภาษณ์พ้นพนักงาน (Exit Interview Log)
            </h4>
            <p className="text-[12px]">
              แผนกบุคคลมีเป้ามุ่งลดปัญหาการอัตราลาออกและพัฒนาบริษัทให้ดียิ่งขึ้น รวบรวมข้อมูลผ่านอินเทอร์เฟซแบบไดนามิกเพื่อบันทึกเหตุผล ความไม่พึงพอใจ และแง่มุมความพึงพอใจภาพรวมที่ต้องการผลักดัน เพื่อประเมินทิศทางแบบเชิงลึก
            </p>
          </section>
        </div>
        
        <div className="p-4 bg-[#f8f9fa] border-t border-[#eaeaec] flex justify-end shrink-0">
          <button onClick={onClose} className="px-8 py-2.5 bg-[#212c46] text-white font-black rounded-xl uppercase text-[12px] hover:bg-[#414757] hover:text-white transition-all shadow-md tracking-[0.1em] cursor-pointer">รับทราบข้อมูล (Understand)</button>
        </div>
      </div>
    </>, document.body
  );
        ...
        >
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden bg-[#f8f9fa] min-h-[460px]">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 bg-white">
                    ...
                    {modalStep === 1 && (
                        <div className="space-y-4 animate-fadeIn font-bold">
...
export default function Offboarding() {
  ...
  return (
    <div className="flex flex-1 w-full flex-col animate-fadeIn bg-transparent space-y-4 px-4 md:px-8 py-6">
