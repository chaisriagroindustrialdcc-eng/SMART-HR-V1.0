    ...
    <>
      ...
      <div className={`fixed inset-y-0 right-0 z-[200] w-full md:w-[500px] bg-white shadow-2xl transform transition-transform duration-500 ease-in-out flex flex-col border-l-2 border-[#b7a159] ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex justify-between items-center p-5 px-6 border-b-2 border-[#b7a159] bg-[#212c46] text-white shrink-0">
          <div>
            <h3 className="font-black flex items-center gap-3 uppercase tracking-widest text-lg"><Icons.BookOpen size={22} className="text-[#b7a159]"/> ROSTER DIRECTIVE</h3>
            <p className="text-[12px] font-bold text-[#d7d7d7] uppercase tracking-widest mt-1.5">Departmental Shift Policies</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 text-[#414757] text-[12px] leading-relaxed custom-scrollbar bg-white">
          <section className="animate-fadeIn">
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Icons.ShieldAlert size={18} className="text-[#b7a159]"/> 1. นโยบายการจัดกะเหมากลุ่ม (Hierarchy Mode)
            </h4>
            <p className="text-[12px] mb-3">
              ระบบนี้ได้รับการออกแบบสำหรับการลงกะในระดับโครงสร้าง <b>หน่วยผลิต/แผนก (Group/Team Roster)</b> โดยไม่มีรายชื่อเฉพาะบุคคล เพื่อควบคุมกำลังพลสายผลิตของ ชัยศรีอะโกรอินดัสเทรียล อย่างครบวงจร มีความยืดหยุ่นสูง และสอดคล้องกับโครงสร้าง 3 ระดับ:
            </p>
            <ul className="list-disc pl-5 space-y-2 text-[12px]">
                <li><strong className="text-[#4d87a8]">Level 1 - Department (ฝ่าย):</strong> เช่น ฝ่ายผลิต (Production), ฝ่ายตรวจสอบคุณภาพ (QA)</li>
                <li><strong className="text-[#b58c4f]">Level 2 - Section (แผนก):</strong> เช่น แผนกจัดเตรียม, บรรจุ หรือฝ่ายห้องปฏิบัติการทดลอง</li>
                <li><strong className="text-[#657f4d]">Level 3 - Group (ชุดกะทำงาน):</strong> เช่น Cook Line 1, Pack Team A ซึ่งเป็นกลุ่มรับสั่งการหลัก</li>
            </ul>
          </section>
          
          <section className="animate-fadeIn" style={{ animationDelay: '0.1s' }}>
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Icons.Clock size={18} className="text-[#d96245]"/> 2. รายละเอียดเวลาการทำงาน (Roster Shifts)
            </h4>
            <p className="text-[12px] mb-3">การจัดตารางเวลาประกอบด้วยกะมาตรฐาน 4 รูปแบบหลัก:</p>
            <div className="grid grid-cols-2 gap-2 text-[11px] mt-2 font-semibold">
                <div className="p-3 bg-[#fdfaf1] border border-[#e8dbb9] rounded-xl text-[#b58c4f]">☀️ Morning Shift (08:00 - 17:00)</div>
                <div className="p-3 bg-[#fcf4f2] border border-[#eedbe2] rounded-xl text-[#d96245]">🌇 Afternoon Shift (14:00 - 23:00)</div>
                <div className="p-3 bg-[#f0f4f8] border border-[#c5dbe5] rounded-xl text-[#212c46]">🌙 Night Shift (22:00 - 07:00)</div>
                <div className="p-3 bg-[#f3f3f1] border border-[#d1d1d5] rounded-xl text-[#7a8b95]">☕ Day Off (Rest Day)</div>
            </div>
          </section>

          <section className="animate-fadeIn" style={{ animationDelay: '0.2s' }}>
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Icons.RefreshCw size={18} className="text-[#3f809e]"/> 3. สถานะตารางและการกระจายงาน (Publish Flow)
            </h4>
            <p className="text-[12px]">
              ตารางงานที่ถูกอัปเดตและมีสถานะเป็น <b>Published</b> จะแจ้งเตือนตารางการทำงานไปยังสมาชิกผู้ปฏิบัติงานใน Group นั้นแบบเรียลไทม์ ตารางดราฟต์จะถูกเก็บเป็นเสมือนโครงสร้างกำลังพลคาดการณ์ล่วงหน้า (Draft Planning) สำหรับหัวหน้าหน้างาน
            </p>
          </section>
        </div>
      ...
      <div className="px-4 sm:px-8 w-full mt-[2px]">
        <div className="w-full space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-3 shrink-0">
                <KpiCard label="Group Schedules" value={totalGroupShifts} icon="calendar-days" colorAccent={THEME.primaryLight} colorValue={THEME.primary} desc="Rostered Slots" />
                <KpiCard label="Distinct Groups" value={uniqueGroups} icon="workflow" colorAccent={THEME.gold} colorValue={THEME.primary} desc="Active Teams" />
                <KpiCard label="Confirmed Shifts" value={publishedShifts} icon="check-circle" colorAccent={THEME.success} colorValue={THEME.success} desc="Published Roster" />
                <KpiCard label="Draft Shifts" value={draftShifts} icon="alert-triangle" colorAccent={draftShifts > 0 ? THEME.danger : THEME.dustyBlue} colorValue={THEME.primary} desc="Draft Planning" />
            </div>
