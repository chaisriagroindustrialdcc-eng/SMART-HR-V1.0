...
const UserGuidePanel = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  return createPortal(
    <>
      ...
      <div className={`fixed inset-y-0 right-0 z-[200] w-full md:w-[500px] bg-[#f3f3f1] shadow-2xl transform transition-transform duration-500 ease-in-out flex flex-col border-l-4 border-[#3f809e] ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="bg-[#212c46] px-6 py-5 flex justify-between items-center text-[#f3f3f1] shrink-0 shadow-sm relative z-10 border-b-2 border-[#b58c4f]">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 bg-gradient-to-tr from-[#3f809e] to-[#748ea1] rounded-xl flex items-center justify-center text-[#212c46] shadow-inner p-[2px]">
                <div className="w-full h-full bg-[#0F172A] rounded-[10px] flex items-center justify-center">
                    <ClipboardCheck size={18} className="text-[#3f809e]"/>
                </div>
            </div>
            <div>
              <h3 className="text-sm font-black flex items-center gap-2 uppercase tracking-widest leading-none mb-1 drop-shadow-sm">SURVEY GUIDE</h3>
              <p className="text-[10px] font-bold text-[#748b9e] uppercase tracking-widest mt-1 drop-shadow-sm">คู่มือการใช้งานระบบสำหรับแบบสำรวจความสุขและผูกพันพนักงาน</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6 text-[#414757] text-[12px] leading-relaxed bg-white">
          <section className="animate-fadeIn">
            <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Smile size={16} className="text-[#b58c4f]"/> 1. Employee Engagement Survey (การทำแบบสำรวจ)
            </h4>
            <p className="mb-2 text-[#414757] font-medium text-[11.5px]">เครื่องมือสำคัญสำหรับใช้รวบรวมฟีดแบ็ก ระดับความสุข สุขภาวะ และความพึงพอใจโดยรวมของคนในองค์กร:</p>
            <ul className="space-y-2 list-none pl-0 text-[11px] font-medium text-[#606a5f]">
              <li className="flex items-start gap-2 bg-[#f8f9fa] p-2.5 rounded-xl border border-[#eaeaec]">
                <Target size={16} className="shrink-0 text-[#b22026] mt-1"/>
                <div>
                  <strong>Target Engagement:</strong> สามารถใช้วัดความพึงพอใจในทีมและสร้างบรรยากาศบวก เพื่อกระตุ้นประสิทธิภาพอย่างยั่งยืน
                </div>
              </li>
              <li className="flex items-start gap-2 bg-[#f8f9fa] p-2.5 rounded-xl border border-[#eaeaec]">
                <BarChart3 size={16} className="shrink-0 text-[#508660] mt-1"/>
                <div>
                  <strong>eNPS Calculation:</strong> Net Promoter Score คำนวณเสียงกลุ่มสนับสนุน (Promoters) หักลบกลุ่มคัดค้าน (Detractors) เพื่อให้เห็นสัดส่วนความผูกพันแท้จริง
                </div>
              </li>
            </ul>
          </section>
          ...
          <section className="animate-fadeIn" style={{ animationDelay: '0.1s' }}>
            <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <BrainCircuit size={16} className="text-[#3f809e]"/> 2. AI Copilot Integration (เชื่อมส่งประเมินผล)
            </h4>
            <div className="bg-[#f0f9f6] p-3.5 rounded-2xl border border-[#e2f0ee] space-y-2 text-[11px] text-[#606a5f] font-medium">
              <p className="text-[#212c46] font-black uppercase">Turnover Risk AI Synergy:</p>
              <p>• เมื่อพนักงานกรอกข้อมูลในระบบ คะแนนความสุขสะสมจะถูกวิเคราะห์รวมเป็นรายกลุ่ม</p>
              <p>• หากคะแนนเฉลี่ยเฉียดระดับวิกฤต (ต่ำกว่า 60%) ระบบ AI จะซิงค์ข้อมูลกับฝ่าย Attrition Analysis โดยตรงเพื่อเตือนภัยอัตราลาออกล่วงหน้า (Predictive Turnover Risk Alert)</p>
            </div>
          </section>

          <section className="animate-fadeIn" style={{ animationDelay: '0.2s' }}>
            <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Settings size={16} className="text-[#7a8b95]"/> 3. Operation Control (การใช้คำสั่งควบคุม)
            </h4>
            <div className="grid grid-cols-1 gap-2.5">
              {[
                { icon: Edit3, label: 'Manage / Edit Survey', desc: 'หน้าเปลี่ยนคำถาม, แก้ไขรายละเอียดช่วงวันที่ และปรับระดับความถี่ในการทำแบบประเมิน', color: '#b58c4f' },
                { icon: ArrowUpRight, label: 'Analytics Dashboard', desc: 'เปิดดูรีพอร์ตสรุปฟีดแบ็กเป็นกราฟสรุปใจความสำคัญ แยกตามพฤติกรรมและระดับตำแหน่ง', color: '#3f809e' },
                { icon: Trash2, label: 'Archive Survey Node', desc: 'ยกเลิกหรือย้ายประวัติแบบประเมินเก่าเพื่อรักษาฐานข้อมูลให้กระชับและลีนอยู่เสมอ ทั้งนี้ประวัติสถิติจะถูกเก็บสำรองไว้อย่างปลอดภัย', color: '#b22026' },
              ].map((btn, idx) => (
          ...
          <section className="animate-fadeIn" style={{ animationDelay: '0.3s' }}>
             <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
               <Info size={16} className="text-[#3f809e]"/> 4. Lifecycle System Status
             </h4>
             <p className="bg-[#3f809e]/5 text-[#2f2926] p-3 rounded-xl border border-[#3f809e]/20 font-medium text-[11px] leading-relaxed">
               สถานะการทำแบบสำรวจ: <strong>Draft</strong> สำหรับจัดทำคำถามเบื้องต้น, <strong>Published</strong> เป็นช่วงส่งกระจายฟอร์มให้พนักงานทำ และ <strong>Closed</strong> เป็นขั้นตอนปิดล็อคฟอร์มเพื่อสรุปผลนำส่งบอร์ดบริหารต่อไป
             </p>
          </section>
        </div>
      ...
      <div className="px-4 sm:px-8 w-full mt-[2px]">
        <div className="w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-3 shrink-0">
                <KpiCard label="Engagement Score" value="78.5%" icon={Smile} colorAccent={THEME.palette.forest} desc="Average Score" />
                <KpiCard label="Employee nPS" value="+24" icon={Users} colorAccent={THEME.palette.cerulean} desc="Advocacy Level" />
                <KpiCard label="AI Risk Analysis" value={surveys.filter(s => s.score < 60 && s.aiSync).length} icon={BrainCircuit} colorAccent={THEME.palette.burntOrange} desc="Turnover Alert" />
                <KpiCard label="Feedbacks Recv." value="250" icon={MessageSquare} colorAccent={THEME.palette.plum} desc="Inbound Insights" />
            </div>
