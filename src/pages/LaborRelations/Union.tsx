  ...
  return createPortal(
    <>
      ...
      <div className={`fixed inset-y-0 right-0 z-[200] w-full md:w-[500px] bg-[#f3f3f1] shadow-2xl transform transition-transform duration-500 ease-in-out flex flex-col border-l-4 border-[#b22026] ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="bg-[#212c46] px-6 py-5 flex justify-between items-center text-[#f3f3f1] shrink-0 shadow-sm relative z-10 border-b-2 border-[#b22026]">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 bg-gradient-to-tr from-[#b58c4f] to-[#f3e5ab] rounded-xl flex items-center justify-center text-[#212c46] shadow-inner p-[2px]">
                <div className="w-full h-full bg-[#0F172A] rounded-[10px] flex items-center justify-center">
                    <Sparkles size={18} className="text-[#b22026]"/>
                </div>
            </div>
            <div>
              <h3 className="text-sm font-black flex items-center gap-2 uppercase tracking-widest leading-none mb-1 drop-shadow-sm">RELATIONS & GRIEVANCES GUIDE</h3>
              <p className="text-[10px] font-bold text-[#748b9e] uppercase tracking-widest mt-1 drop-shadow-sm">คู่มือระบบบริหารแรงงานสัมพันธ์และพิจารณาสอบสวนข้อร้องเรียน</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6 text-[#414757] text-[12px] leading-relaxed bg-white">
          <section className="animate-fadeIn">
            <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Scale size={16} className="text-[#b22026]"/> 1. Complaint & Union Node (การจัดการสหภาพแรงงาน)
            </h4>
            <p className="mb-2 text-[#414757] font-medium text-[11.5px]">แผงควบคุมหลักเพื่อขับเคลื่อนนโยบายแรงงานสัมพันธ์อย่างเท่าเทียมและถูกหลักกฎหมายแรงงาน:</p>
            <ul className="space-y-2 list-none pl-0 text-[11px] font-medium text-[#606a5f]">
              <li className="flex items-start gap-2 bg-[#f8f9fa] p-2.5 rounded-xl border border-[#eaeaec]">
                <Users size={16} className="shrink-0 text-[#3f809e] mt-1"/>
                <div>
                  <strong>Union Representative Channel:</strong> ช่องทางการเจรจา ประสานงาน และสืบสานข้อหารือร่วมกับตัวแทนสหภาพพนักงานอย่างเป็นรูปธรรม
                </div>
              </li>
              <li className="flex items-start gap-2 bg-[#f8f9fa] p-2.5 rounded-xl border border-[#eaeaec]">
                <ShieldCheck size={16} className="shrink-0 text-[#508660] mt-1"/>
                <div>
                  <strong>Fact Investigation:</strong> ทุกประเด็นร้องเรียนจะมีขั้นตอนส่งพิสูจน์ข้อเท็จจริงอย่างสุจริต โดยสามารถแนบหลักฐานอ้างอิงเพื่อใช้ในการสืบสวนคดี
                </div>
              </li>
            </ul>
          </section>
          ...
          <section className="animate-fadeIn" style={{ animationDelay: '0.1s' }}>
            <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <AlertTriangle size={16} className="text-[#b22026]"/> 2. Severity Escalation Protocols (ระดับความร้ายแรง)
            </h4>
            <div className="bg-[#fff5f5] p-3.5 rounded-2xl border border-[#ffe3e3] space-y-2 text-[11px] text-[#606a5f] font-medium">
              <p className="text-[#b22026] font-black uppercase flex items-center gap-1.5"><AlertTriangle size={14}/> Critical Protection Node:</p>
              <p>• <strong>High (ร้ายแรง):</strong> ประเด็นที่มีความล่อแหลม และอาจส่งผลเสียต่อภาพรวมความมั่นคงของบริษัท จะมีการส่งสตรีมเตือนผู้ใหญ่ในระบบทันที</p>
              <p>• <strong>Medium / Low:</strong> การร้องเรียนด้านวินัยความประพฤติ สภาพสิ่งแวดล้อม และข้อสงสัยด้านสิทธิ์สวัสดิการประจำวัน</p>
            </div>
          </section>

          <section className="animate-fadeIn" style={{ animationDelay: '0.2s' }}>
            <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Hammer size={16} className="text-[#b58c4f]"/> 3. Core Action Flow (คีย์บุ่มระบบสืบสวน)
            </h4>
            <div className="grid grid-cols-1 gap-2.5">
              {[
                { icon: Edit3, label: 'Investigate Detail Node', desc: 'เปิดแผงบันทึกรายละเอียด วิเคราะห์พยาน แหล่งที่มา สรุปเนื้อหาประเด็น และอัปเดตสถานะคดีสอบสวน', color: '#b58c4f' },
                { icon: Trash2, label: 'Archive Record Node', desc: 'เสร็จสิ้นการแก้ปัญหา นำกรณีที่คุ้มครองเสร็จแล้วไปจัดเก็บเข้าฐานข้อมูลเก่าถาวร เพื่อรักษาความเป็นส่วนตัวของผู้เกี่ยวข้อง', color: '#b22026' },
              ].map((btn, idx) => (
          ...
          <section className="animate-fadeIn" style={{ animationDelay: '0.3s' }}>
             <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
               <Info size={16} className="text-[#3f809e]"/> 4. Audit Security Compliance
             </h4>
             <p className="bg-[#3f809e]/5 text-[#2f2926] p-3 rounded-xl border border-[#3f809e]/20 font-medium text-[11px] leading-relaxed">
               ตามมาตรฐานความโปร่งใสทางระบบบุคคล (TamarindPro Compliance Protocol) ข้อมูลผู้ใช้และประเด็นร้องทุกข์ทั้งหมดจะมีความปลอดภัยสูงสุดภายใต้ พรบ. คุ้มครองข้อมูลส่วนบุคคล (PDPA)
             </p>
          </section>
        </div>
      ...
      <div className="px-4 sm:px-8 w-full mt-[2px]">
        <div className="w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-3 shrink-0">
                <KpiCard label="Open Grievances" value={grievances.filter(g=>g.status!=='Resolved').length} icon={AlertTriangle} colorAccent={THEME.palette.red} desc="Awaiting Action" />
                <KpiCard label="Resolved Cases" value={grievances.filter(g=>g.status==='Resolved').length} icon={ShieldCheck} colorAccent={THEME.palette.forest} desc="Resolution Success" />
                <KpiCard label="Union Proposals" value={grievances.filter(g=>g.reporter==='สหภาพแรงงานฯ').length} icon={Hammer} colorAccent={THEME.palette.cerulean} desc="Labor Proposals" />
                <KpiCard label="Avg. Response" value="1.2 days" icon={Users} colorAccent={THEME.palette.slate} desc="Target Service Node" />
            </div>
