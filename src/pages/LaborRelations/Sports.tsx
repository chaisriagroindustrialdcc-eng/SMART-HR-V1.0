...
// High Quality, Detailed User Guide Panel mimicking the standards in UserPermissions
const UserGuidePanel = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  return createPortal(
    <>
      ...
      <div className={`fixed inset-y-0 right-0 z-[200] w-full md:w-[500px] bg-[#f3f3f1] shadow-2xl transform transition-transform duration-500 ease-in-out flex flex-col border-l-4 border-[#b58c4f] ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 bg-gradient-to-tr from-[#b58c4f] to-[#f3e5ab] rounded-xl flex items-center justify-center text-[#212c46] shadow-inner p-[2px]">
                <div className="w-full h-full bg-[#0F172A] rounded-[10px] flex items-center justify-center">
                    <Sparkles size={18} className="text-[#b58c4f]"/>
                </div>
            </div>
            <div>
              <h3 className="text-sm font-black flex items-center gap-2 uppercase tracking-widest leading-none mb-1 drop-shadow-sm">ACTIVITY CONTROL GUIDE</h3>
              <p className="text-[10px] font-bold text-[#748b9e] uppercase tracking-widest mt-1 drop-shadow-sm">คู่มือการใช้งานระบบจัดการกิจกรรมและสวัสดิการส่งเสริมสัมพันธ์</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6 text-[#414757] text-[12px] leading-relaxed bg-white">
          <section className="animate-fadeIn">
            <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Trophy size={16} className="text-[#b58c4f]"/> 1. Category Nodes (การแยกหมวดหมู่กิจกรรมหลัก)
            </h4>
            <p className="mb-2 text-[#414757] font-medium text-[11.5px]">แผงควบคุมระบบจัดหมวดหมู่กลุ่มกิจกรรมส่งเสริมสุขภาพ วัฒนธรรมองค์กร และการประเมิน Reach Rate:</p>
            <ul className="space-y-2 list-none pl-0 text-[11px] font-medium text-[#606a5f]">
              <li className="flex items-start gap-2 bg-[#f8f9fa] p-2.5 rounded-xl border border-[#eaeaec]">
                <Users size={16} className="shrink-0 text-[#b22026] mt-1"/>
                <div>
                  <strong>Sports & Social:</strong> กิจกรรมนันทนาการ กีฬา และสวัสดิการสังสรรค์ภายใน มีเป้าหมายส่งสร้าง Team Dynamics ในออฟฟิศ
                </div>
              </li>
              <li className="flex items-start gap-2 bg-[#f8f9fa] p-2.5 rounded-xl border border-[#eaeaec]">
                <Globe size={16} className="shrink-0 text-[#508660] mt-1"/>
                <div>
                  <strong>External / CSR:</strong> แผนงานรวมผลประโยชน์ภายนอก กิจกรรมสังคม สิ่งแวดล้อม และพันธมิตรความร่วมมือทางอุตสาหกรรม
                </div>
              </li>
              <li className="flex items-start gap-2 bg-[#f8f9fa] p-2.5 rounded-xl border border-[#eaeaec]">
                <Megaphone size={16} className="shrink-0 text-[#3f809e] mt-1"/>
                <div>
                  <strong>Internal PR & Dispatch:</strong> คอยดูแลระบบการกระจายข่าวสารถึงพนักงานทางโทรศัพท์มือถือ เพื่อวัดระดับความตื่นตัวของโครงสร้างสังคมภายใน
                </div>
              </li>
            </ul>
          </section>
          ...
          <section className="animate-fadeIn" style={{ animationDelay: '0.1s' }}>
            <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <DollarSign size={16} className="text-[#508660]"/> 2. Budget Synchronization (การวิเคราะห์งบประมาณจัดสรร)
            </h4>
            <div className="bg-[#f0f9f6] p-3.5 rounded-2xl border border-[#e2f0ee] space-y-2 text-[11px] text-[#606a5f] font-medium">
              <p className="text-[#212c46] font-black uppercase">Welfare & Budget allocation Sync:</p>
              <p>• ยอดจัดสรรงบประมาณ (Budget Location Node) จะถูกซิงค์ข้อมูลส่งต่อไปยังแผนกบัญชีและสวัสดิการกลางของบริษัทโดยตรง</p>
              <p>• พนักงานที่เข้าร่วมกิจกรรมจะสะสมแต้มความสุข และสร้างค่าสัมประสิทธิ์สถิติทางบวก เพื่อรายงานตรงต่อบอร์ดบริหาร</p>
            </div>
          </section>

          <section className="animate-fadeIn" style={{ animationDelay: '0.2s' }}>
            <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Settings size={16} className="text-[#7a8b95]"/> 3. Interactive Controls (คีย์ปุ่มความคุมหลัก)
            </h4>
            <div className="grid grid-cols-1 gap-2.5">
              {[
                { icon: Edit3, label: 'Edit Activity Detail', desc: 'ปรับแต่งรายละเอียดกิจกรรม วันที่กำหนดการ ยอดงบประมาณ แผนกลุ่มเป้าหมาย และเป้ารอยยิ้มพนักงาน', color: '#b58c4f' },
                { icon: Send, label: 'PR Dispatch Notify', desc: 'ดำเนินการแพร่ประกาศข่าวสารเชื่อมส่งเข้าสู่ TamarindPro Mobile App และบันทึกประวัติการส่งอย่างเป็นทางการ', color: '#3f809e' },
                { icon: Trash2, label: 'Archive Activity Unit', desc: 'ยกเลิกหรือย้ายประกาศเก่าเข้าคลังข้อมูลประวัติเพื่อลดความแออัดของระบบ โดยโครงสร้างฐานข้อมูลจะถูกเก็บเซฟสำรองอย่างปลอดภัย', color: '#b22026' },
              ].map((btn, idx) => (
          ...
          <section className="animate-fadeIn" style={{ animationDelay: '0.3s' }}>
             <h4 className="text-[13px] font-black text-[#2f2926] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
               <Info size={16} className="text-[#3f809e]"/> 4. Lifecycle Milestones & States
             </h4>
             <p className="bg-[#3f809e]/5 text-[#2f2926] p-3 rounded-xl border border-[#3f809e]/20 font-medium text-[11px] leading-relaxed">
               สถานะการดำเนินงาน: <strong>Planned</strong> สำหรับกิจกรรมที่อยู่ระหว่างการขออนุมัติงบ, <strong>In Progress</strong> สำหรับช่วงลุยโครงการ, <strong>Released / Active</strong> ประชาสัมพันธ์แล้ว และ <strong>Completed</strong> เพื่อสรุปรายงานและปิดงบดำเนินการ
             </p>
          </section>
        </div>
      ...
      <div className="px-4 sm:px-8 w-full mt-[2px]">
        <div className="w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-3 shrink-0">
                <KpiCard label="Activity Nodes" value={events.length} icon={ClipboardCheck} colorAccent={THEME.palette.charcoal} desc="Total Managed" />
                <KpiCard label="Total Budget" value={`${(events.reduce((acc,e)=>acc+e.budget, 0)/1000).toFixed(1)}K`} icon={DollarSign} colorAccent={THEME.palette.forest} desc="Budget Allocated" />
                <KpiCard label="Employee Reach" value="94.5%" icon={Megaphone} colorAccent={THEME.palette.burntOrange} desc="Communications PR" />
                <KpiCard label="Registered Attendees" value={events.reduce((acc,e)=>acc+e.participants,0)} icon={Users} colorAccent={THEME.palette.cerulean} desc="Active Personnel" />
            </div>
