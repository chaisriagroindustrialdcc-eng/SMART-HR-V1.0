...
const KpiCard = ({ icon: IconComp, value, label, colorAccent, colorValue, desc }: any) => (
    <div className="bg-white/90 px-5 py-4 rounded-xl border border-[#eaeaec] shadow-sm flex-1 min-w-[200px] relative overflow-hidden group hover:border-[#b58c4f] transition-all min-h-[100px] flex flex-col justify-between animate-fadeIn">
        ...
        <div className="relative z-10 mt-1 flex items-end justify-between">
            <p className="text-[24px] font-black leading-none text-[#212c46] font-mono" style={{color: colorValue}}>
                {value}
            </p>
    ...
    <>
      ...
      <div className={`fixed inset-y-0 right-0 z-[200] w-full md:w-[500px] bg-white shadow-2xl transform transition-transform duration-500 ease-in-out flex flex-col border-l-2 border-[#b58c4f] ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex justify-between items-center p-5 px-6 border-b-2 border-[#b58c4f] bg-[#212c46] text-white shrink-0">
          <div>
            <h3 className="font-black flex items-center gap-3 uppercase tracking-widest text-lg"><Icons.BookOpen size={22} className="text-[#b58c4f]"/> PAYROLL GUIDE</h3>
            <p className="text-[12px] font-bold text-[#d7d7d7] uppercase tracking-widest mt-1.5">Payroll Engine & Calculations</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 text-[#414757] text-[12px] leading-relaxed custom-scrollbar bg-white font-sans">
          <section className="animate-fadeIn">
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Icons.Zap size={18} className="text-[#3f809e]"/> 1. Auto-Sync Mechanics
            </h4>
            <p className="text-[12px] mb-3">ระบบคำนวณเงินเดือนทำงานสัมพันธ์รวบรวมข้อมูลจากโมดูลอื่นๆ อัตโนมัติ (Automated Pipelines):</p>
            <ul className="list-none pl-0 space-y-3">
                <li className="flex items-start gap-2 bg-[#f8f9fa] p-3 rounded-xl border border-[#eaeaec]">
                  <Icons.Users size={16} className="shrink-0 text-[#212c46] mt-0.5"/> 
                  <div><strong className="text-[#212c46]">Salary Master:</strong> อิงอัตราเงินเดือนหลักและค่าครองชีพตามสัญญาจ้างหลักในแฟ้มพนักงาน</div>
                </li>
                <li className="flex items-start gap-2 bg-[#f8f9fa] p-3 rounded-xl border border-[#eaeaec]">
                  <Icons.Clock size={16} className="shrink-0 text-[#3f809e] mt-0.5"/> 
                  <div><strong className="text-[#3f809e]">Time & Attendance:</strong> สรุปชั่วโมงล่วงเวลาพิเศษ (OT) และจำนวนสาย/ขาดงานแบบ Real-time</div>
                </li>
                <li className="flex items-start gap-2 bg-[#932c2e]/10 p-3 rounded-xl border border-[#932c2e]/30">
                  <Icons.Calendar size={16} className="shrink-0 text-[#932c2e] mt-0.5"/> 
                  <div><strong className="text-[#932c2e]">Leave System:</strong> ซิงค์วันลากิจลากิจสะสม และการลาไร้เบี้ยรับกรณีพิเศษ (Unpaid Leave)</div>
                </li>
            </ul>
          </section>
          ...
          <section className="animate-fadeIn" style={{ animationDelay: '0.1s' }}>
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Icons.Play size={18} className="text-[#b58c4f]"/> 2. Execution Pipelines
            </h4>
            <p className="text-[12px] mb-3">ขั้นตอนการปฏิบัติงานของเจ้าหน้าที่ HR (3-Steps Process Workflow):</p>
            <ul className="list-disc pl-5 mt-2 space-y-2 text-[12px]">
                <li><strong className="text-[#212c46]">Step 1 (Sync & Compute):</strong> เลือกรอบงวดเวลา Period / ประเภท แล้วรันระบบคำนวณเพื่อจำลองประมวลผล</li>
                <li><strong className="text-[#b58c4f]">Step 2 (Adjust Overrides):</strong> ตรวจสอบรายคน และกดแก้ไขค่าเบี้ยเฉพาะคราว (Manual Bonuses & Deductions)</li>
                <li><strong className="text-[#657f4d]">Step 3 (Publish System):</strong> กดอนุมัติปิดรอบบัญชี เพื่อเตรียมพิมพ์ใบจ่ายเงินเดือน (PDF Payslips)</li>
            </ul>
          </section>

          <section className="animate-fadeIn" style={{ animationDelay: '0.2s' }}>
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Icons.ShieldCheck size={18} className="text-[#657f4d]"/> 3. Immutable Security
            </h4>
            <p className="text-[12px]">ระบบจะรักษาความถูกต้องของข้อมูล Master ย้อนหลัง โดยข้อมูลการปรับเงินเดือนในงวดปัจจุบัน <b>จะจัดทำเป็นแบบ Snapshot (ไม่ส่งผลเสียต่อสัญญาจ้างหลัก)</b> เพื่อความปลอดภัยสูงสุด</p>
          </section>
        </div>
...
export default function PayrollCalculation() {
  ...
  const statusCounts = useMemo(() => {
      const periodRecords = records.filter(r => r.periodId === currentPeriodLabel);
      const counts: Record<string, number> = { All: periodRecords.length, Draft: 0, Approved: 0, Paid: 0 };
      periodRecords.forEach(r => { if (r.status in counts) counts[r.status]++; });
      return counts;
  }, [records, currentPeriodLabel]);
      ...
      <div className="px-4 sm:px-8 w-full mt-[2px]">
        <div className="w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-3 shrink-0">
                <KpiCard label="ACTIVE DISBURSALS" value={totalInPeriod} icon={Users} colorAccent={THEME.palette.tealDark} colorValue={THEME.primary} desc="Managed Accounts" />
                <KpiCard label="DRAFTS PENDING" value={draftCount} icon={AlertTriangle} colorAccent={draftCount > 0 ? THEME.palette.coral : THEME.palette.olive} colorValue={draftCount > 0 ? THEME.palette.coral : THEME.primary} desc="Awaiting Verification" />
                <KpiCard label="TOTAL GROSS EXPENSE" value={formatCurrency(totalNetPay)} icon={Banknote} colorAccent={THEME.palette.olive} colorValue={THEME.palette.olive} desc="Payout Projection" />
                <KpiCard label="ENGINE MECHANICAL" value={isProcessing ? "PROCESSING..." : "VERIFIED"} icon={isProcessing ? RefreshCw : ShieldCheck} colorAccent={isProcessing ? THEME.palette.gold : THEME.palette.teal} colorValue={THEME.palette.teal} desc="System Integrated" />
            </div>
            ...
            <div className="bg-white rounded-3xl shadow-lg border border-[#eaeaec] overflow-hidden flex flex-col animate-fadeIn">
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        ...
                        <div className="flex items-center gap-1.5 bg-white border border-[#eaeaec] rounded-full px-4 py-2 shadow-sm">
                            ...
                            <span className="shrink-0 bg-[#212c46] text-white font-mono text-[9px] px-1.5 py-0.5 rounded-full font-black min-w-[20px] text-center">
                                {statusCounts[filterStatus] || statusCounts['All'] || 0}
                            </span>
                        </div>
                    </div>
                ...
                {selectedRecordIds.length > 0 && (
                    <div className="px-8 py-3 bg-amber-50/50 border-b border-[#eaeaec] flex flex-col sm:flex-row items-center justify-between gap-3 font-sans animate-fadeIn">
