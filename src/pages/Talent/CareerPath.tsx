        ...
        <div className="flex justify-between items-center p-5 px-6 border-b-2 border-[#b7a159] bg-[#212c46] text-white shrink-0">
          <div>
            <h3 className="font-black flex items-center gap-3 uppercase tracking-widest text-lg">
              <Compass size={22} className="text-[#b7a159]"/> CAREER PATH GUIDE
            </h3>
            <p className="text-[12px] font-bold text-[#d7d7d7] uppercase tracking-widest mt-1.5 font-mono">Progression & Pathway Directory</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 text-[#414757] text-[12px] leading-relaxed custom-scrollbar bg-white">
          <section className="animate-fadeIn">
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <GitBranch size={18} className="text-[#b7a159]"/> 1. PATHWAYS STEPS & TRANSITION
            </h4>
            <p className="text-[12px] mb-3">เส้นทางความก้าวหน้ารายบุคคล ได้รับการออกแบบเป็นสายงานเชิงโครงสร้างแบบระบุลำดับขั้นสะสม (Linear Progression Nodes):</p>
            <ul className="list-disc pl-5 space-y-2 text-[12px]">
              <li><strong className="text-[#212c46]">Entry Phase:</strong> จุดเริ่มต้นในฐานะผู้จบการศึกษาใหม่ หรือฝ่ายเทคนิคขั้นต้น</li>
              <li><strong className="text-[#4d87a8]">Specialist Node:</strong> การฝึกอบรบจนครบเงื่อนไข SOP และรับภาระพื้นที่รับผิดชอบของตน</li>
              <li><strong className="text-[#b7a159]">Management/Expertise Level:</strong> กำหนดแผนบริหารกระบวนงาน (Approve) ระบุพฤติกรรมการตัดสินใจ</li>
            </ul>
          </section>
          
          <section className="animate-fadeIn" style={{ animationDelay: '0.1s' }}>
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Clock size={18} className="text-[#3f809e]"/> 2. YEAR REQUIREMENTS
            </h4>
            <p className="text-[12px] leading-relaxed">
              การเลื่อนลำดับขั้นในแต่ละ Node จะยึดเกณฑ์ชั่วโมงทดลองงานและเกณฑ์ขั้นต่ำของ <b>ปีปฏิบัติงานสะสม (Minimum Years Target)</b> เพื่อรักษามาตรฐาน ISO และความปลอดภัยสูงสุดของสภาพแวดล้อมปฏิบัติการ
            </p>
          </section>

          <section className="animate-fadeIn" style={{ animationDelay: '0.15s' }}>
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Lock size={18} className="text-[#932c2e]"/> 3. SYNCHRONIZATION AND ACCESS LOCKS
            </h4>
            <p className="text-[12px] font-bold text-[#2f2926] bg-[#932c2e]/10 border border-[#932c2e]/30 p-3 rounded-xl leading-relaxed">
              สิทธิ์ในการประเมินและอนุมัติความสอดคล้อง (Approve) จะถูกควบคุมตามนโยบายรักษาความลับส่วนบุคคล ผู้กุมสิทธิ์ที่ผ่านการตรวจสอบจากกล่อง User Permissions เท่านั้นที่จะได้รับสิทธิ์ปรับแต่งเกณฑ์เหล่านี้
            </p>
          </section>
        </div>
...
const KpiCard = ({ iconName, value, label, colorAccent, colorValue, desc }: any) => {
  const IconComp = iconName === 'BarChart3' ? BarChart3 :
      ...
      <div className="px-4 sm:px-8 w-full mt-[2px]">
        <div className="w-full">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-3 shrink-0">
            <KpiCard label="Average Succession Years" value={`${kpiData.avgYears} YRS`} iconName="Clock" colorAccent={THEME.gold} colorValue={THEME.primary} desc="Lower Bound Milestone" />
            <KpiCard label="Defined Career Roadmaps" value={kpiData.total} iconName="GitBranch" colorAccent={THEME.skyBlue} colorValue={THEME.primary} desc="Active SOP Pipelines" />
            <KpiCard label="Fully Verified Pipelines" value={kpiData.activeLength} iconName="BarChart3" colorAccent={THEME.success} colorValue={THEME.success} desc="No Broken Gaps Found" />
          </div>
