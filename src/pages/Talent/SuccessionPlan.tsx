...
const Toast = ({ message, type = 'success', onClose }: any) => (
  createPortal(
    <div className="fixed bottom-6 right-6 z-[1000] animate-slideInRight flex items-center gap-3 bg-white px-5 py-3.5 rounded-xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-[#eaeaec]">
      ...
      <button onClick={onClose} className="ml-4 text-[#7a8b95] hover:text-[#2f2926] cursor-pointer transition-colors"><X size={14} /></button>
    </div>,
    document.body
  )
);

// --- Comprehensive User Guide matching UserPermissions ---
    ...
    <>
      ...
      <div className={`fixed inset-y-0 right-0 z-[200] w-full md:w-[500px] bg-white shadow-2xl transform transition-transform duration-500 ease-in-out flex flex-col border-l-2 border-[#b7a159] ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex justify-between items-center p-5 px-6 border-b-2 border-[#b7a159] bg-[#212c46] text-white shrink-0">
          <div>
            <h3 className="font-black flex items-center gap-3 uppercase tracking-widest text-lg">
              <Network size={22} className="text-[#b7a159]"/> SUCCESSION GUIDE
            </h3>
            <p className="text-[12px] font-bold text-[#d7d7d7] uppercase tracking-widest mt-1.5 font-mono">Talent Succession Directory</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 text-[#414757] text-[12px] leading-relaxed custom-scrollbar bg-white">
          <section className="animate-fadeIn">
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Briefcase size={18} className="text-[#b7a159]"/> 1. CRITICAL SUCCESSION ROLES
            </h4>
            <p className="text-[12px] mb-3">ระบบประเมินความเสี่ยงและแผนสืบทอดตำแหน่งงานที่สำคัญยิ่งยวด (Critical Roles) ได้รับการออกแบบตามความสอดคล้องระดับความเสี่ยงการรักษาบุคลากร (Retention Risk):</p>
            <ul className="list-disc pl-5 space-y-2 text-[12px]">
              <li><strong className="text-[#212c46]">Low Risk:</strong> ความต่อเนื่องของตำแหน่งงานยังมั่นคง มีอัตราการคงอยู่ระดับดีเลิศ</li>
              <li><strong className="text-[#a94228]">Medium Risk:</strong> ตัวบ่งชี้ความพึงพอใจเริ่มปรับตัว จำเป็นต้องเริ่มคัดกรองผู้สืบทอดสำรอง</li>
              <li><strong className="text-[#932c2e]">High Risk:</strong> ความเสี่ยงสูงสุด จำเป็นต้องเปิดกระบวนงานสืบทอดฉุกเฉินและประสานทักษะเร่งด่วน</li>
            </ul>
          </section>
          
          <section className="animate-fadeIn" style={{ animationDelay: '0.1s' }}>
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Clock size={18} className="text-[#3f809e]"/> 2. READINESS TIMELINE CATEGORY
            </h4>
            <p className="text-[12px] leading-relaxed mb-3">
              รอบเวลาความพร้อมของผู้สืบทอดตำแหน่ง (Successor Readiness Node) แบ่งออกเป็น 4 ระดับ เพื่อการจัดทัพทักษะอย่างลงตัว:
            </p>
            <div className="space-y-2">
              <div className="bg-[#657f4d]/10 p-2.5 rounded-lg border border-[#657f4d]/20 flex items-center gap-2">
                <span className="w-2.5 h-2.5 rounded-full bg-[#657f4d]" />
                <span className="font-extrabold text-[11px] text-[#212c46]">Ready Now</span> - พร้อมสืบทอดภายในระยะเวลาทันทีโดยไร้ข้อจำกัด
              </div>
              <div className="bg-[#3f809e]/10 p-2.5 rounded-lg border border-[#3f809e]/20 flex items-center gap-2">
                <span className="w-2.5 h-2.5 rounded-full bg-[#3f809e]" />
                <span className="font-extrabold text-[11px] text-[#212c46]">1-2 Years</span> - ทักษะพร้อม แต่ยังต้องพัฒนาระบบสนับสนุนเพิ่มเติม
              </div>
              <div className="bg-[#b58c4f]/10 p-2.5 rounded-lg border border-[#b58c4f]/20 flex items-center gap-2">
                <span className="w-2.5 h-2.5 rounded-full bg-[#b58c4f]" />
                <span className="font-extrabold text-[11px] text-[#212c46]">3-5 Years</span> - อยู่ในขั้นตอนสะสมชั่วโมงงานและใบรับรองสายวิชาชีพ
              </div>
            </div>
          </section>
          ...
          <section className="animate-fadeIn" style={{ animationDelay: '0.15s' }}>
            <h4 className="text-[14px] font-black text-[#212c46] mb-3 uppercase flex items-center gap-2 border-b-2 border-[#d7d7d7] pb-2 font-mono">
              <Lock size={18} className="text-[#932c2e]"/> 3. STABILITY AND ACCESS LOCKS
            </h4>
            <p className="text-[12px] font-bold text-[#2f2926] bg-[#932c2e]/10 border border-[#932c2e]/30 p-3 rounded-xl leading-relaxed">
              ผู้ตรวจสอบระดับ Verifier และผู้มีอำนาจตัดสินใจระดับสูงจากหน้า User Permissions เท่านั้นที่จะได้รับการเข้าถึงโปรโตคอลแผนสืบทอดนี้ เพื่อรักษาความลับขั้นสูงสุดของยุทธศาสตร์บุคลากรองค์กร
            </p>
          </section>
        </div>
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-[#eaeaec] flex justify-end shrink-0">
          <button onClick={onClose} className="px-6 py-2.5 bg-[#212c46] text-[#eaeaec] font-black rounded-xl uppercase text-[11px] hover:bg-[#b7a159] hover:text-white transition-all shadow-md tracking-widest border border-transparent cursor-pointer">
            เข้าใจข้อกำหนดทั้งหมด (Got it)
          </button>
        </div>
      </div>
    </>, document.body
...
// --- Lean KPI Card with ultra tight padding ---
const KpiCard = ({ iconComp: IconComp, value, label, colorAccent, colorValue, desc }: any) => {
  return (
    <div className="bg-white px-4 py-3 rounded-2xl border border-[#eaeaec] shadow-sm flex-1 min-w-[200px] relative overflow-hidden group hover:border-[#b7a159] transition-all min-h-[90px] flex flex-col justify-between animate-fadeIn cursor-default">
      ...
      <div className="px-4 sm:px-8 w-full mt-[2px]">
        <div className="w-full">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-3 shrink-0">
            <KpiCard label="Key Positions" value={kpiData.total} iconComp={Briefcase} colorAccent={THEME.skyBlue} colorValue={THEME.primary} desc="Managed Registry" />
            <KpiCard label="Critical Vacant" value={kpiData.critical - kpiData.covered} iconComp={Target} colorAccent={THEME.accent} colorValue={THEME.primary} desc="Urgent Fill Gates" />
            <KpiCard label="Ready Successors" value={kpiData.covered} iconComp={UserCheck} colorAccent={THEME.success} colorValue={THEME.primary} desc="Active Pipeline" />
            <KpiCard label="Position At-risk" value={kpiData.atRisk} iconComp={ShieldAlert} colorAccent={THEME.gold} colorValue={THEME.gold} desc="High Attrition Gaps" />
          </div>
