...
import { Clock, Calendar, Box, Target, Activity, ChevronLeft, ChevronRight, Bell, Eye, Printer, Shield, Cloud, RefreshCw } from 'lucide-react';
...
export default function Header({ onOpenPendingPanel }: HeaderProps = {}) {
  ...
  useEffect(() => {
    const checkWatermark = () => {
      setIsWatermarkActive(document.body.classList.contains('has-print-watermark'));
    };
    window.addEventListener('watermark-state-changed', checkWatermark as EventListener);
    // Initial check
    checkWatermark();
    return () => window.removeEventListener('watermark-state-changed', checkWatermark as EventListener);
  }, []);

  const handleToggleWatermark = () => {
    if ((window as any).toggleConfidentialWatermark) {
      (window as any).toggleConfidentialWatermark();
    }
  };
  ...
  return (
    <header className={`h-24 px-8 flex flex-row items-center justify-between z-10 shrink-0 bg-transparent w-full transition-all duration-300 ${isModalOpen ? 'filter blur-[12px] opacity-40 pointer-events-none scale-[0.98]' : ''}`}>
      ...
      <div className="flex items-center gap-3.5">
          ...
          <div 
            className="flex items-center bg-white shadow-sm rounded-full border border-[#cdd0db]/50 h-11 shrink-0 px-3.5 gap-2.5 hover:border-[#b58c4f]/40 transition-all duration-200 select-none cursor-help"
          ...
          <div className="hidden md:flex items-center">
            <motion.div 
              layout
              className="flex items-center bg-white shadow-sm rounded-full border border-[#cdd0db]/50 h-11 shrink-0 overflow-hidden pr-2 transition-colors duration-200"
              animate={{ 
            ...
            >
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0" title={copilotStatus === 'active' ? 'AI Copilot Engine Connected' : 'AI Copilot Connection Limited'}>
                        ...
                        <div className="flex flex-col text-left uppercase whitespace-nowrap">
                            <span className="text-[8px] font-black text-slate-400 tracking-widest leading-none">COPILOT SERVICE</span>
                    ...
                    {/* Main HR Database Status */}
                    <div className="flex items-center gap-2 shrink-0" title={dbStatus === 'active' ? 'Main HR Central Google Sheet Connected' : 'Google Sheets SCRIPT_URL not found. Running in Local Mode.'}>
                        <div className="relative flex items-center justify-center">
                        ...
                        <div className="flex flex-col text-left uppercase whitespace-nowrap">
                            <span className="text-[8px] font-black text-slate-400 tracking-widest leading-none">CENTRAL DB</span>
                            <span className="text-[9px] font-black text-[#212c46] mt-0.5 leading-none">
                                {dbStatus === 'checking' ? 'CONNECTING...' : dbStatus === 'active' ? 'GAS ONLINE' : 'LOCAL BACKUP'}
                            </span>
                        </div>
                    </div>
                  </motion.div>
              ...
              <button 
                ...
                title={isExpanded ? "ยับรายละเอียดกลับ" : "คลี่รายละเอียดออก"}
              >
          ...
          {/* Confidential Watermark Support */}
          <button
            onClick={handleToggleWatermark}
            className={`no-print relative h-11 px-4 rounded-full border transition-all gap-2 shrink-0 cursor-pointer group flex items-center justify-center ${
              isWatermarkActive 
                ? 'bg-rose-50 text-rose-700 border-rose-300 shadow-sm' 
                : 'bg-white text-slate-500 border-[#cdd0db]/50 hover:bg-slate-50 hover:border-rose-350'
            }`}
            title="คลิกเพื่อเปิด/ปิด ลายน้ำเอกสารลับ CONFIDENTIAL บนหน้าเอกสารพิมพ์ย้อนหลัง"
          >
            <Shield size={14} className={`transition-all ${isWatermarkActive ? 'text-rose-600 scale-110' : 'text-slate-400 group-hover:scale-105'}`} />
            <span className="hidden lg:inline-block leading-none font-sans font-black uppercase text-[9px] mt-0.5">
              {isWatermarkActive ? 'Watermark ON' : 'Watermark OFF'}
            </span>
          </button>

          {/* Print Preview Quick Action */}
          <button
            onClick={() => window.print()}
            className="no-print relative h-11 px-4 rounded-full bg-white border border-[#cdd0db]/50 shadow-sm flex items-center justify-center text-[#212c46] hover:bg-slate-50 hover:border-[#b58c4f]/40 transition-all gap-2 shrink-0 cursor-pointer group"
            title="คลิกเพื่อพิมพ์รายงานหน้าปัจจุบันเป็น PDF หรือพิมพ์กระดาษ (Print Preview)"
          >
            <Eye size={14} className="text-[#b58c4f] group-hover:scale-105 transition-all" />
